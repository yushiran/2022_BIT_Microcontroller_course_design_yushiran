	...
	switch(button_value)
		{
			...
			case 2: //2表示up被按下
					...
					break;
			case 6: //6表示button_b被按下
					restart_game_flag=1;//游戏暂停标志位置1,游戏暂停
					break;
			default:;
		}	
...
unsigned char basic_button()
{
	...
	if(button_b==0)
	{
		if(button_b_reg<button_delay)//按键一直被按下时设置时间间隔触发
		{
			button_b_reg++;
		}
		else
		{
			button_b_reg=0;
			tpflag=6;//返回3表示button_b被按下
		}		
	}
	else
	{
		button_b_reg=button_delay;//释放按键时置按键缓存为button_delay，以便在下次按键时及时响应
	}
